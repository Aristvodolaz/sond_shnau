# 🚀 Развертывание проекта

## Что реализовано

✅ **База данных PostgreSQL**
- Подключение к вашей БД (155.212.216.148:5432)
- 7 таблиц: users, sessions, dogs, news, stories, memorial, adopted
- Миграция данных из JSON

✅ **Админ-панель**
- Вход/выход (сессии)
- Управление собаками (CRUD)
- Управление новостями (CRUD)
- Красивый интерфейс

✅ **Улучшенный UI**
- Продвинутые компоненты (UiInput, UiTextarea, UiModal, UiLoader, UiToast)
- Анимации появления элементов
- Hover эффекты на карточках
- Градиенты и микроанимации
- Кастомный scrollbar

✅ **API Endpoints**
- Публичные: GET /api/dogs, GET /api/news, GET /api/rss
- Админские: полный CRUD для dogs и news
- Аутентификация через cookies

---

## 📝 Шаг за шагом

### 1. Установка зависимостей

```bash
npm install
```

**Установится:**
- `pg` — PostgreSQL драйвер
- `zod` — валидация
- `rss-parser` — парсинг RSS
- И остальные зависимости

### 2. Настройка подключения к БД

Откройте `server/database/config.ts` и проверьте настройки:

```typescript
export const dbConfig = {
  host: '155.212.216.148',
  port: 5432,
  database: 'fond_shnau',
  user: 'my_user',
  password: '123456',
  ssl: false
}
```

Если нужно изменить — отредактируйте этот файл.

### 3. Запуск dev-сервера

```bash
npm run dev
```

Сайт запустится на: http://localhost:3000

### 4. Инициализация базы данных

После запуска сервера откройте в браузере:

```
http://localhost:3000/api/init-db
```

Эта страница:
1. Создаст все таблицы в PostgreSQL
2. Перенесёт данные из JSON файлов (dogs.json, news.json и т.д.)
3. Создаст админа (login: `admin`, password: `admin123`)

**Вы увидите JSON ответ:**
```json
{
  "success": true,
  "message": "Database initialized successfully",
  "steps": [
    "Tables created",
    "Data migrated from JSON",
    "Admin user created (username: admin, password: admin123)"
  ]
}
```

**⚠️ ВАЖНО:** Вызывайте этот endpoint только один раз! При повторном вызове данные НЕ задублируются благодаря `ON CONFLICT DO NOTHING`.

### 5. Вход в админку

1. Откройте: http://localhost:3000/admin/login
2. Логин: `admin`
3. Пароль: `admin123`

**После первого входа обязательно смените пароль!**

---

## 🎨 Что можно настроить

### 1. Контакты и ссылки

`utils/config.ts`:
```typescript
VK_URL: 'https://vk.com/ваша_группа'
TG_URL: 'https://t.me/ваш_канал'
PHONE: '+7 (XXX) XXX-XX-XX'
EMAIL: 'info@example.com'
```

### 2. Цветовая схема

`tailwind.config.ts` → секция `theme.extend.colors`:
```typescript
primary: { /* оранжевые оттенки */ }
warm: { /* бежевые оттенки */ }
```

### 3. Данные

Теперь все данные в PostgreSQL! Управляйте через админку:
- http://localhost:3000/admin — собаки
- http://localhost:3000/admin/news — новости

---

## 🗄️ Структура БД

### Таблицы

1. **users** — админы
   - id, username, password, created_at

2. **sessions** — сессии для авторизации
   - id, token, user_id, expires_at, created_at

3. **dogs** — собаки
   - id, slug, name, type, age, city, curator_name, curator_phone, curator_email
   - photos (JSONB), description, features (JSONB), health, character
   - forum_topic_url, status, date_added, created_at, updated_at

4. **news** — новости
   - id, slug, title, date, preview, content, image, published
   - created_at, updated_at

5. **stories** — счастливые истории
   - id, slug, dog_name, year, part, preview, content
   - photos (JSONB), before_photo, after_photo, created_at, updated_at

6. **memorial** — память
   - id, slug, name, years, type, photo, tribute, date_added, created_at

7. **adopted** — пристроенные собаки
   - id, name, type, year, city, photo, adoption_date, created_at

---

## 🔐 Безопасность

### Сменить пароль админа

1. Войдите в админку
2. (Функция смены пароля будет добавлена позже)

Временно: измените в `server/utils/auth.ts` функцию `ensureAdminUser()`, удалите запись из таблицы `users` и запустите `npm run db:init` снова.

### Продакшн-рекомендации

1. **Используйте bcrypt** для хеширования паролей (сейчас SHA-256)
2. **Включите SSL** для PostgreSQL
3. **Используйте переменные окружения** вместо хардкода паролей
4. **Настройте HTTPS** для сайта
5. **Ограничьте доступ к БД** по IP

---

## 📡 API Endpoints

### Публичные

```
GET  /api/dogs              # Все собаки
GET  /api/news              # Опубликованные новости
GET  /api/rss               # RSS лента (proxy)
GET  /rss.xml               # RSS XML
```

### Админские (требуют cookie `admin_token`)

```
POST   /api/admin/login     # Вход
POST   /api/admin/logout    # Выход

GET    /api/admin/dogs      # Все собаки
POST   /api/admin/dogs      # Добавить собаку
PUT    /api/admin/dogs/:id  # Обновить собаку
DELETE /api/admin/dogs/:id  # Удалить собаку

GET    /api/admin/news      # Все новости
POST   /api/admin/news      # Добавить новость
PUT    /api/admin/news/:id  # Обновить новость
DELETE /api/admin/news/:id  # Удалить новость
```

---

## 🐛 Troubleshooting

### Ошибка native binding (oxc-parser)

```
ERROR Cannot find native binding. npm has a bug related to optional dependencies
```

**Решение:**
```bash
cd /root/sond_shnau
rm -rf node_modules package-lock.json
npm cache clean --force
npm install --ignore-scripts
npm run postinstall
```

### Ошибка PostCSS "Cannot use 'import.meta' outside a module"

```
ERROR Pre-transform error: [postcss] Cannot use 'import.meta' outside a module
```

**Причина:** Старая версия Node.js (< 18) или отсутствие PostCSS конфига

**Решение 1 - Обновить Node.js (рекомендуется):**
```bash
# Установить nvm
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.0/install.sh | bash
source ~/.bashrc

# Установить Node 20
nvm install 20
nvm use 20
nvm alias default 20

# Проверить версию
node -v  # Должно быть v20.x.x

# Переустановить зависимости
cd /root/sond_shnau
rm -rf node_modules package-lock.json
npm install
```

**Решение 2 - Если Node.js >= 18 но ошибка есть:**
```bash
# Просто обновите зависимости
cd /root/sond_shnau
npm install --force
```

**Примечание:** Nuxt 3 автоматически настраивает PostCSS. Внешний `postcss.config.cjs` не нужен и может вызывать предупреждения.

### Ошибка подключения к БД

```
Error: connect ECONNREFUSED
```

**Решение:**
1. Проверьте что PostgreSQL запущен
2. Проверьте настройки в `server/database/config.ts`
3. Проверьте firewall и доступ к порту 5432

### Ошибка при миграции

```
Error: duplicate key value violates unique constraint
```

**Решение:**
- Данные уже есть в БД
- Удалите таблицы и запустите `npm run db:init` снова
- Или используйте `ON CONFLICT DO NOTHING` (уже есть в коде)

### Не могу войти в админку

**Проверьте:**
1. База данных инициализирована (`npm run db:init`)
2. Логин: `admin`, пароль: `admin123`
3. Cookies не заблокированы
4. Сессия не истекла (7 дней)

---

## 🎉 Готово!

Проект полностью готов к работе:
- ✅ База данных PostgreSQL
- ✅ Админ-панель с CRUD
- ✅ Красивый современный UI
- ✅ RSS лента
- ✅ Все разделы сайта

**Команды для запуска:**
```bash
npm install          # Один раз
npm run dev          # Запустить сервер
```

**Затем откройте в браузере:**
```
http://localhost:3000/api/init-db  # Один раз для инициализации БД
```

**После инициализации:**
- **Сайт:** http://localhost:3000
- **Админка:** http://localhost:3000/admin/login (admin / admin123)

Приятной работы! 🐕✨
