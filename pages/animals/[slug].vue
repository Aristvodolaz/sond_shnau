    ...
    <template v-else>
      ...
      <div class="container-custom pt-6 pb-0">
        <nav class="flex items-center gap-2 text-sm text-warm-500 flex-wrap">
          ...
          <NuxtLink to="/animals" class="hover:text-primary-600 transition-colors">Животные</NuxtLink>
          <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
          </svg>
      ...
      <div class="container-custom py-8 md:py-10">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 xl:gap-16">
          ...
          <div>
            ...
            <div class="bg-white rounded-2xl border border-warm-100 p-5 mb-5">
              ...
              <div class="grid grid-cols-2 gap-2.5">
                ...
                <a
                  ...
                  class="flex items-center justify-center gap-2 py-3 px-4 bg-green-50 border border-green-200 rounded-xl text-sm font-semibold text-green-800 hover:bg-green-100 transition-colors"
                >
            ...
            <p v-if="dog.forumTopicUrl" class="mt-4 text-xs text-warm-400">
              <a :href="dog.forumTopicUrl" target="_blank" rel="noopener noreferrer" class="hover:text-primary-500 transition-colors underline">
      ...
      <div class="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-warm-200 px-4 py-3 safe-bottom">
        <div class="flex gap-2">
          ...
          <a :href="waHref" target="_blank" rel="noopener noreferrer" class="w-12 h-12 flex items-center justify-center rounded-xl bg-green-50 border border-green-200 text-green-700 hover:bg-green-100 transition-colors shrink-0">
...
const { waUrl, tgUrl } = useContactLinks()
...
const waHref = computed(() => {
  if (!dog.value) return '#'
  const n = dog.value.curator.whatsapp || dog.value.curator.phone
  return waUrl(n, `Здравствуйте! Вопрос по ${dog.value.name}`)
})
